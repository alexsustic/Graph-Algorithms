...
class Graph(object):
    ...
    def getMinimalCostPath(self,first_vertex,second_vertex,previous,distance):
        path = []
    ...
    def minimumCostWalk(self,first_vertex,second_vertex):  
        ...
        path = self.getMinimalCostPath(first_vertex,second_vertex,previous,distance)    
    ...
    def topologicalSort(self):
        ...
        queue = []
        count  = {}
        for vertex in range(0, self.get_nr_of_vertices()):
            count[vertex] = self.get_in_degree(vertex)
            if count[vertex] == 0 :
                queue.append(vertex);
        while(len(queue) != 0):
            queue_vertex = queue.pop(0)
            sorted.append(queue_vertex)
            for next_vertex in self.parse_out_neighbours(queue_vertex):
                count[next_vertex] = count[next_vertex] - 1
                if (count[next_vertex] == 0):
                    queue.append(next_vertex)
        if (len(sorted) < self.get_nr_of_vertices()):
            sorted.clear()
        return sorted
    
    
    def highestCostPath(self,source, destination):
        topologicalSorted = self.topologicalSort()
        distance = {}
        path= {}
        for i in range(0, self.get_nr_of_vertices()):
            distance[i] = -11111
        ...
        distance[source] = 0;
        path[source] = 0
        for vertex in topologicalSorted:
            if(vertex == destination):
                break
            for out_neighbour in self.parse_out_neighbours(vertex):
                if distance[out_neighbour] < distance[vertex] + self.__dictionaryCosts[(vertex,out_neighbour)]:
                    distance[out_neighbour] = distance[vertex] + self.__dictionaryCosts[(vertex,out_neighbour)]
                    path[out_neighbour] = vertex
